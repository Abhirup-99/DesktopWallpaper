...
def scrape(end=1, start=1, term=""):
    ...
    page_soup = soup(requests.get(url).content, "html.parser")
...
def get_url(img_page):
    page_soup = soup(requests.get(img_page).content, "html5lib")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Scrape wallpapers from WallpaperFlare."
    )
